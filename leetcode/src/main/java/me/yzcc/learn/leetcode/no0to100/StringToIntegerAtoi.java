        ...
        for (int i = 0; i < inputs.length; i++) {
            ...
            if (state.equals(State.BLANK)) {
                ...
                if (isNumber(curr)) {
                    int value = curr - '0';
                    if (signal) {
                        if ((Integer.MAX_VALUE / 10 < result) || (Integer.MAX_VALUE - result * 10) < value) {
                            return Integer.MAX_VALUE;
                        } else {
                            result = result * 10 + value;
                        }
                    } else {
                        // 0 - Integer.MIN_VALUE 会溢出
//                        if (result == 0) {
//                            result = result * 10 - value;
//                        } else
                        if ((Integer.MIN_VALUE / 10 > result) || (result * 10 - Integer.MIN_VALUE) < value) {
